...
use diesel::{query_dsl::methods::{LimitDsl, SelectDsl}, ExpressionMethods, RunQueryDsl, SelectableHelper};
use serde::{Serialize, Deserialize};
use warp::reject::InvalidQuery;

use crate::{db::DBConnection, error::{InternalServerError, InvalidQueryError, InvalidSessionError}, models::Post, schema::posts::{self, timestamp}, validate_session_from_headers};
...
pub async fn public_space_query(headers: warp::http::HeaderMap, connection: DBConnection, query: HashMap<String, String>) -> Result<impl warp::Reply, warp::Rejection> {
    ...
    let posts: Vec<Post> = diesel::QueryDsl::order_by(
        diesel::query_dsl::methods::OffsetDsl::offset(
            posts::table.select(Post::as_select()), 20 * page as i64),
            timestamp.desc()
        )
        .limit(20)
